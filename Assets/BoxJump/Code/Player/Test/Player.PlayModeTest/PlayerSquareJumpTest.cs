...
namespace BoxJump.Code.Player.Test.Player.PlayModeTest
{
    public class PlayerSquareJumpTest
    {
        ...
        public IEnumerator SquareFallsOut_TouchTriggerWithPositiveHeight_PlayerDies(float distanceBetweenDeathFloorAndPlayer)
        {
            //Arrange
            GameObject deathGroundObject = _squareGameObject.transform.GetChild(0).gameObject;
            deathGroundObject.transform.position = new Vector3(0, _squareGameObject.transform.position.y - distanceBetweenDeathFloorAndPlayer);
            yield return null;
            ...
            Assert.IsTrue(!playerSquareDeath.IsDead == (deathGroundObject.transform.position.y <= _squareGameObject.transform.position.y));
